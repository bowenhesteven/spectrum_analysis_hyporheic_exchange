/*
* ========================================================================
* mexMultiSlope.c - C MEX file to compute upslope contributing area
*             using the multi-slope algorithm
*
*     usage: [A, B] = mexMultiSlope(M,dy/dx,flood,ndv)
*
*     Input arguments:
...
void D8Dir(const int K, const int J, double M[], double Bdy[], double D[], double W[], double minima[], double *a, int *numMin)
{

int i, j, k, Kj, Kjup, Kjdown, theD;
double invdx, invdy, invdiag, theS, maxS, Stot;
double e0;
  ...
  for (j=0; j<J; j++) { // Loop through columns
      ...
      for (i=0; i<K; i++) { // Loop through rows
          ...
          if (Bdy[Kj+i] > 0) { // if it is nodata, skip it
              ...
              Stot = 0; // set total slope to zero
              theD = 0;
              ...
              for (k=0; k<numfacets; k++) { // Loop through facets, ignoring any that have nodata
                  ...
                  // if it's a downslope cell, set the MS weight equal to the slope. We'll normalize later.
                  if (theS>0) { 
                      W[k*J*K+Kj+i] = theS;
                      Stot += theS;
                  }
                  
                  // If this is a downslope cell AND the steepest yet, record its slope and direction
                  if (theS>maxS) {
                      maxS=theS;
                      theD=facetidx[k]+1;
                  } 
              ...
              // Normalize MS weights by sum of slopes
              for (k=0; k<8; k++) {
                  if (W[k*J*K+Kj+i] > 0) {
                      W[k*J*K+Kj+i] = W[k*J*K+Kj+i]/Stot;
                  }
              }
          }
      } 
...
void GetMSArea(const int i, const int j, const int K, const int J, double A[], double W[], double B[], double Bdy[])
{
...
for (k=0; k<8; k++) {  // loop through drainage directions
    ...
    if (p>=0 && p<K && q>=0 && q<J && Bdy[K*q+p]>0) {  // avoid going off boundaries or into nodata regions
        ...
        if ((theW=W[widx])>0) {  // if the current drainage direction has a weight 
            GetMSArea(p,q,K,J,A,W,B,Bdy);  // recursive call to get drainage area for neighbor k 
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
  ...
  *numMin = 0;
  ...
  D8Dir(K,J,M,Bdy,D,W,minima,a,numMin); // Calculate D8 drainage directions
...
//       D8Dir(K,J,M,Bdy,D,minima,a,numMin); // Calculate D8 drainage directions
  ...
  for (i=0; i<K; i++) {
    for (j=0; j<J; j++) {
      GetMSArea(i,j,K,J,A,W,B,Bdy); // Dinf
    }
  }
