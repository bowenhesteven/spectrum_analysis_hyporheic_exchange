function [P ,Pm ,fm ,Pv ,fv] = fft2D(M, dx, dy, pad, window)
...
% not given
if (nargin < 2) || (nargout > 5), help(mfilename), return, end
...
if pad
    ...
    Ly = Lx; % Make the area square;
else % no zero padding
    Lx = nx;
    Ly = ny;
end

% M_windowed=M; % Output the detrended windowed topography;
...
P = fftshift(fft2(M,Ly,Lx));% truncates M or pads 
% with trailing zeros to form an Ly-by-Lx matrix before computing the transform. 
% fft2 is Ly-by-Lx. return a 2-dimensional fast fourier transform
% rearranges a Fourier transform X by shifting the zero-frequency component to the center of the array.
P(Ly/2 + 1, Lx/2 + 1)=0; % Although the mean of the detrended matrix is 
...
M = P .* conj(P) / (Lx * Ly * Wss); % DFT periodogram;

% assign the power spectrum to the output argument
Pm = M; % Pm is the DFT periodogram; output;
...
M = M(:,1:(Lx/2+1)); % DFT periodogram;
% fvec = dfreq*sqrt((rows(:,1:xc)-yc).^2 + (cols(:,1:xc)-xc).^2);
fv = fm(:,1:(Lx/2+1)); % frequency matrix;

% Why sort thid half-column out?
...
fv = sortrows(horzcat(fv(:),M(:)),1); % concatenate column vectors of 
                                      ...
                                      % frequency which is the first column
                                      % "1" and the DFT periodogram is the
                                      % column "2"
...
Pv = 2*fv(:,2); % Second column is the DFT periodogram;
% the factor of 2 corrects for the fact that we have
                % taken only half of the 2D spectrum. sum(Pvec) should
                % now equal sum(Pmat(:)).
fv = fv(:,1); % The sorted radial frequency;
end
